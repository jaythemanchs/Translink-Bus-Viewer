...
function toDate(n) {
 ...
 class Buffer {
	 ...
	 */
	slice(start, end) {
		return end ? this.buffer.subarray(start, end) : this.buffer.subarray(this.offset, this.offset += start)
	}
...
class Message {
	 ...
	 */
	newClass(c) {
		const result = new c(this.buffer, Number(this.buffer.readVarInt()))
...
class FeedHeader extends Message {
 ...
 class FeedEntity extends Message {
	parse(index, _, def) {
		switch (index) {
			case 1:
...
class StopTimeEvent extends Message {
	parse(index, _, def) {
		switch (index) {
			case 1:
				this.delay = def()
				if (this.delay >> 63n) this.delay = Number((this.delay & 0x7FFFFFFFFFFFFFFFn) - 0x8000000000000000n)
				else this.delay = Number(this.delay)
				break
			case 2:
				this.timestamp = toDate(def())
				break
